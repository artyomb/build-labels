...
`trivy-runner` scans locally built images using Docker/Buildx and Trivy installed on PATH. It requires the metadata file produced by the build:
...
`--fail-on` accepts comma-separated `UNKNOWN,LOW,MEDIUM,HIGH,CRITICAL`, ignoring case; the default is `HIGH,CRITICAL`. Scanning covers vulnerabilities, including unfixed findings under Trivy's default configuration. Trivy's environment/configuration controls its cache, database access, and ignore rules.
