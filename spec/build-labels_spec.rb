...
describe 'Test BuildLabels' do
  it 'build-labels' do
    ...
    response = `bundle exec ./bin/build-labels -c ./examples/simple-compose.yml --cache-to=type=local,dest=./.cache,mode=max --cache-from=type=local,src=./.cache gitlab cache to_dockerfiles to_compose`
    puts response
